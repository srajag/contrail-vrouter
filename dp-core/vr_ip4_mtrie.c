        ...
        struct vr_packet *pkt)
{
    ...
    struct ip4_bucket  *bkt;
    struct ip4_bucket_entry *ent;
    struct ip4_mtrie_cache_entry *cent;
    int idx;
    ...
    cent = &table->cache[idx];
    if (rt->rtr_req.rtr_prefix == cent->prefix) {
        rt->rtr_req.rtr_label_flags = cent->label_flags;
        rt->rtr_req.rtr_label = cent->label;
        rt->rtr_req.rtr_prefix_len = cent->prefix_len;

        return cent->nexthop_p;
    }
#endif
...
#if (MTRIE_CACHE_SIZE > 0)
            /* set cache entry */
            cent->nexthop_p = PTR_TO_NEXTHOP(ptr);
            cent->prefix = rt->rtr_req.rtr_prefix;
            cent->label_flags = rt->rtr_req.rtr_label_flags;
            cent->label = rt->rtr_req.rtr_label;
            cent->prefix_len = rt->rtr_req.rtr_prefix_len;
#endif
