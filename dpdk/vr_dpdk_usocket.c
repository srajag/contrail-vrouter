...
vr_usocket_connect(struct vr_usocket *usockp)
{
    ...
    strncpy(sun.sun_path, VR_PACKET_AGENT_UNIX_FILE, sizeof(sun.sun_path));
...
vr_usocket_bind(struct vr_usocket *usockp)
{
    ...
    case UNIX:
        sun.sun_family = AF_UNIX;
        bzero(sun.sun_path, sizeof(sun.sun_path));
        strncpy(sun.sun_path, VR_NETLINK_UNIX_FILE, sizeof(sun.sun_path));
    ...
    case RAW:
        ...
        sun.sun_family = AF_UNIX;
        bzero(sun.sun_path, sizeof(sun.sun_path));
        strncpy(sun.sun_path, VR_PACKET_UNIX_FILE, sizeof(sun.sun_path));
        addr = (struct sockaddr *)&sun;
        addrlen = sizeof(sun);
        server = false;
